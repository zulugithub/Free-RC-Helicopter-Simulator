...
namespace IO_Heli_X
{
    ...
    public class Heli_X_Import
	{
        ...
        static IEnumerator Download_Progress(UnityWebRequest req, string scenery_name, MonoBehaviour justToStartCoroutine, GameObject ui_canvas)
		{
            while (!req.isDone)
			{
                // find the trigger button by its name (the button is destroyed during updating the ui, thus no direct connetction to the button-object worked), and add the download percentage value to its text
